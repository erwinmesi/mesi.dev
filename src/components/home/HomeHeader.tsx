...
import Pipe from './SkillPipe';

function HomeHeader() {
  return (
    <section className="flex gap-y-6 sm:gap-x-8 items-center my-12 flex-col sm:flex-row max-w-2xl mx-auto">
      <span>
        <Image src={image} width="128" height="128" alt="Erwin Mesias" />
      </span>
      <div className="space-y-1">
        <h1 className="text-center sm:text-left text-2xl">Erwin Mesias</h1>
        <h2 className="text-center sm:text-left text-grey-200 text-base">
          Full-Stack Web & Mobile Developer
        </h2>
        <h3 className="text-center sm:text-left text-grey-300 hover:text-grey-300 hover:underline flex items-center justify-center sm:justify-start">
        ...
        <p className="text-center sm:text-left text-sm">
          PHP <Pipe /> Laravel <Pipe /> Vue.js <Pipe /> Nuxt.js <Pipe /> React.js <Pipe /> Next.js
        </p>
      </div>
    </section>
  );
